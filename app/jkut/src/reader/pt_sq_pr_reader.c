...
Exp *pt_sq_pr_reader_read (Exp *left, Cdr *cdr) {
  if (cdr_next_token_is_point(cdr)) { // . ------------------------------------
    ...
    if (exp_is_sym(exp)) {
      ...
      return pt_sq_pr_reader_read(exp_pt(left, exp, js), cdr);
    }
  ...
  if (cdr_next_token_is_exclamation(cdr)) { // ! -------------------------------
    ...
    return pt_sq_pr_reader_read(exp_sq(left, exp_int(0, "0"), js), cdr);
  }
  ...
  if (cdr_next_token_is_open_square(cdr)) { // [ -------------------------------
    ...
    if (cdr_next_token_is_colon(cdr)) {
      ...
      if (cdr_next_token_is_close_square(cdr)) { // [:]
        ...
        return pt_sq_pr_reader_read(
          exp_slice(left, opt_none(), opt_none(), js), cdr
        );
      }
      ...
      if (token_is_close_square(tk)) { // [:e]
        ...
        return pt_sq_pr_reader_read(
          exp_slice(left, opt_none(), opt_some(exp2), js), cdr
        );
      }
    ...
    if (cdr_next_token_is_close_square(cdr)) { // [e]
      ...
      return pt_sq_pr_reader_read(exp_sq(left, exp1, js), cdr);
    }
    if (cdr_next_token_is_colon(cdr)) {
      Token *colon = cdr_read_token(cdr);
      if (cdr_next_token_is_close_square(cdr)) { // [e:]
        ...
        return pt_sq_pr_reader_read(
          exp_slice(left, opt_some(exp1), opt_none(), js), cdr
        );
      }
      Exp *exp2 = ex_reader_read(cdr);
      Token *tk = cdr_read_token(cdr);
      if (token_is_close_square(tk)) { // [e1:e2]
        ...
        return pt_sq_pr_reader_read(
          exp_slice(left, opt_some(exp1), opt_some(exp2), js), cdr
        );
      }
      EXC_GENERIC(cdr_fail_expect(cdr, "']'", token_to_str(tk)));
    }
  ...
  if (cdr_next_token_is_open_parenthesis(cdr)) { // ( --------------------------
    ...
    return pt_sq_pr_reader_read(exp_pr(left, exps, buf_str(js)), cdr);
  }
