...
int finanzas_read_raw(Arr **codes_new, Darr **qs_new) {
  char *path =  "http://www.finanzas.com/mercado-continuo/";
  // Varr[char]
  ...
  // Table -------------------------------------------------
  varr_push(tstart, "<div class=\"mod-valores-completo\">");
...
int finanzas_read(Varr **nicks_new, Darr **qs_new) {
  ...
  *qs_new = qs;
  ...
  int e = finanzas_read_raw(&codes, &qcs);
  if (e) {
    return e;
  }
  ...
  while (*pnks) {
    ...
    if (!*code) {
      io_loge("Nick '%s' not found in %s", nk, finanzas_name());
      return 1;
    }
    ...
    _EACH
    if (q == -1) {
      io_loge("Quote of '%s' not fund in %s", nk, finanzas_name());
      return 1;
    }
  }
