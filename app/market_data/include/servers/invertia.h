...
#ifndef SERVERS_INVERTIA_H
  #define SERVERS_INVERTIA_H

#include "dmc/std.h"
#include "dmc/Darr.h"

/// Returns the invertia server name.
char *invertia_name(void);

// ONLY FOR DEBUG
// invertia_read_raw works the same as 'read' but returning server codes
// instead nicks.
// Returns 1 if reading is wrong, otherwise returns 0
int invertia_read_raw(Arr **codes_new, Darr **qs_new, char *path);

/// 'invertia_read' reads current quotes in new pairs Varr[char] - Darr.<p>
