...
#ifndef SERVERS_BOLSAMANIA_H
  #define SERVERS_BOLSAMANIA_H

#include "dmc/std.h"
#include "dmc/Darr.h"

/// Returns the bolsamania server name.
char *bolsamania_name(void);

// ONLY FOR DEBUG
// bolsamania_read_raw works the same as 'read' but returning server codes
// instead nicks.
// Returns 1 if reading is wrong, otherwise returns 0
int bolsamania_read_raw(Arr **codes_new, Darr **qs_new);

/// 'bolsamania_read' reads current quotes in new pairs Varr[char] - Darr.<p>
