...
static struct _Follow *_follow_new(
  ...
  this->nick = nick;
  this->calc = wmacalc2_new(closes_short_size, closes_long_size, QUOTES_NUMBER);
  this->inc = -10.0;
  return this;
}

struct followwma_FollowWma {
  Gen *interval; // 5 + v
  Gen *len_short; // 5 + v
  Gen *len_long; // 5 + v
  Gen *level; // %
...
typedef struct followwma_FollowWma FollowWma;

static FollowWma *new(
  Gen *interval,
  Gen *len_short,
  Gen *len_long,
  Gen *level
) {
  FollowWma *this = MALLOC(FollowWma);
...
static bool gen_eq(FollowWma *this, FollowWma *other) {
...
static void prepare(FollowWma *this) {
  if (this->extra) {
...
static void reset(FollowWma *this) {
    this->extra = NULL;
}
...
static void process(
  ...
  if (!wmacalc2_add(&follow->inc, follow->calc, close)) {
    return;
  }
...
static Json *trace_data(FollowWma *this, size_t nick) {
  struct _Follow *follow = arr_get(this->extra, nick);

  Arr/*Json*/ *jsr = arr_new();

  jarr_adouble(jsr, wmacalc2_short_sum(follow->calc), 4);
  jarr_adouble(jsr, wmacalc2_long_sum(follow->calc), 4);
  jarr_adouble(jsr, follow->inc, 4);

  return json_warray(jsr);
}

static FollowWma *mutate(FollowWma *this) {
...
static Json *serialize(FollowWma *this) {
  Arr/*Json*/ *a = arr_new();
...
static Flea *mk_fextra(Flea *f, FollowWma *this) {
  return flea_fextra(
    f,
    this,
