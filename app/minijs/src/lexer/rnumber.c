...
static Tx *ddigits0(Tx *tx, Buf *bf, char *chs) {
  Tx *r;
  char d;
  while (tx_neq(tx, r = token_csplit0(&d, tx, chs))) {
    tx = r;
    if (d != '_') {
      buf_cadd(bf, d);
    }
...
static Tx *ddigits(Tx *tx, Buf *bf, char *chs) {
  ...
  if (tx_neq(tx, r = token_cconst(tx, 't'))) {
    buf_cadd(bf, 't');
    return r;
  } else {
...
static Tx *rradix(Value **v, Tx *tx) {
  ...
  if (tx_eq(tx, r = token_cconst0(tx, '0'))) {
    return tx;
  }
  tx = r;
  ...
  if (tx_neq(tx, r = token_cconst0(tx, '.'))) {
    return start;
  } else if (tx_neq(tx, r = token_cconst0(tx, 'b'))) {
    buf_add(bf, "0b");
    tx = ddigits(r, bf, BIN);
  } else if (tx_neq(tx, r = token_cconst0(tx, 'x'))) {
    buf_add(bf, "0x");
    tx = ddigits(r, bf, HEX);
  } else if (tx_neq(tx, r = token_cconst0(tx, 't'))) {
    buf_add(bf, "0t");
    tx = r;
  } else if (tx_eq(tx, r = token_csplit0(&d, tx, OCT))) {
    buf_add(bf, "0");
  } else {
    buf_add(bf, "0o");
...
static Tx *rfloat(Tx *tx, Buf *bf) {
  ...
  char d;
  if (tx_eq(tx, r = token_csplit0(&d, tx, "eE"))) {
    return token_blanks(tx);
  }
  tx = r;
  buf_cadd(bf, 'e');

  if (tx_neq(tx, r = token_csplit0(&d, tx, "-+"))) {
    tx = r;
    buf_cadd(bf, d);
  }
...
Tx *rnumber(Value **v, Tx *tx) {
  ...
  Buf *bf = buf_new();
  char d;
  if (tx_eq(tx, r = token_csplit(&d, tx, DEC))) {
    return tx;
  }
  ...
  if (tx_neq(tx, r = token_csplit0(&d, tx, ".eE"))) { // Float
    tx = rfloat(tx, bf);
