...
static Exp *read_switch (Types *tps, Cdr *cdr, char *blanks) {
  ...
  for (;;) {
    ...
    if (exp_is_sym(c1) && !strcmp(exp_get_sym(c1), "default")) {
      ...
      return exp_switch(cond, cases, str_f("(%s)", buf_str(bf)));
    }
...
static Exp *read_new (Types *tps, Cdr *cdr, char *blanks) {
  Buf *js = buf_new();
  buf_add(js, blanks);
  buf_add(js, "new ");
...
static Exp *read_null (Types *tps, Cdr *cdr, char *blanks) {
  Exp *exp = ex_reader_read(tps, cdr);
...
static Exp *read_async (Types *tps, Cdr *cdr, char *blanks) {
  Exp *exp = ex_reader_read(tps, cdr);
...
static Exp *read_await (Types *tps, Cdr *cdr, char *blanks) {
  Exp *exp = ex_reader_read(tps, cdr);
  if (!exp_is_pr(exp))
...
Exp *ex_reader_read (Types *tps, Cdr *cdr) {
  ...
  if (cdr_next_token_is_ternary(cdr)) {
    ...
    if (token_is_colon(tk)) {
      Exp *exp3 = ex_reader_read(tps, cdr);
      char *js = str_f("%s%s%s%s%s",
...
Exp *ex_reader_read1 (Types *tps, Cdr *cdr) {
  ...
  if (token_is_open_square(tk)) { // [ ------------------------------------------
    ...
    if (cdr_next_token_is_colon(cdr)) { // Range
      ...
      if (cdr_next_token_is_colon(cdr)) {
        ...
        exp3 = opt_some(ex_reader_read(tps, cdr));
      }
  ...
  if (token_is_backslash(tk)) { // \ -------------------------------------------
    ...
    if (cdr_next_token_is_arrow(cdr)) {
      ...
      for(;;) {
        ...
        char *module = "";
        if (token_is_colon(tk2)) {
          buf_add(js, str_left(tk2->js, -1));
          tk2 = cdr_read_token(cdr);
          if (tk2->type != TOKEN_SYMBOL)
            EXC_GENERIC(cdr_fail_expect(cdr, "symbol", token_to_str(tk2)));
          module = tk2->value;
          buf_add(js, str_left(tk2->js, -strlen(module)));
          tk2 = cdr_read_token(cdr);
        }

        if (tk2->type == TOKEN_SYMBOL) {
          if (*module)
            types_add(tps, tk2->value, module);
  ...
  if (tk->type == TOKEN_LINE_COMMENT || tk->type == TOKEN_COMMENT) {
    char *js = tk->js;
    Exp *r = ex_reader_read1(tps, cdr); // skip comment
  ...
  if (tk->type == TOKEN_SYMBOL) {
    ...
    if (!strcmp(sym, "switch")) return read_switch(tps, cdr, str_left(tk->js, -6));
    else if (!strcmp(sym, "new")) return read_new(tps, cdr, str_left(tk->js, -3));
    else if (!strcmp(sym, "class")) return read_class(cdr, str_left(tk->js, -5));
    else if (!strcmp(sym, "null")) return read_null(tps, cdr, str_left(tk->js, -4));
    else if (!strcmp(sym, "eval")) return read_eval(cdr, str_left(tk->js, -4));
    else if (!strcmp(sym, "async")) return read_async(tps, cdr, str_left(tk->js, -5));
    else if (!strcmp(sym, "await")) return read_await(tps, cdr, str_left(tk->js, -5));
