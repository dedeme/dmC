...
static void key (Machine *m) {
  ...
  machine_push(m, token_new_string(cryp_key(tx, len)));
}

static void cryp (Machine *m) {
  char *k = tk_pop_string(m);
  char *tx = tk_pop_string(m);
  machine_push(m, token_new_string(cryp_cryp(tx, k)));
}

static void decryp (Machine *m) {
  char *k = tk_pop_string(m);
  char *tx = tk_pop_string(m);
  machine_push(m, token_new_string(cryp_decryp(tx, k)));
}

static void autocryp (Machine *m) {
  Int len = tk_pop_int(m);
  char *tx = tk_pop_string(m);
  machine_push(m, token_new_string(cryp_auto_cryp(tx, len)));
}

static void autodecryp (Machine *m) {
  char *tx = tk_pop_string(m);
  machine_push(m, token_new_string(cryp_auto_decryp(tx)));
}
...
static void encode (Machine *m) {
  ...
  Int len = tk_pop_int(m);
  char *tx = tk_pop_string(m);
  machine_push(m, token_new_string(cryp_encode(tx, len, k)));
}

static void decode (Machine *m) {
  char *k = tk_pop_string(m);
  char *tx = tk_pop_string(m);
  machine_push(m, token_new_string(cryp_decode(tx, k)));
}
