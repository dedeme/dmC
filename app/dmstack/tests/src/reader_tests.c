...
void reader_tests (void) {
  ...
  Reader *r = reader_new("*tests*", "1", 1, 0);
  ...
  r = reader_new("*tests*", "", 1, 0);
  tk = reader_process(r);
  assert(token_type(tk) == token_LIST);
  a = token_list(tk);
  assert(arr_size(a) == 0);

  r = reader_new("*tests*", "  -117  ", 1, 0);
  tk = reader_process(r);
  assert(token_type(tk) == token_LIST);
  a = token_list(tk);
  assert(arr_size(a) == 1);
  ...
  tk = arr_get(a, 0);
  assert(token_type(tk) == token_SYMBOL);
  assert(str_eq(token_symbol(tk), "pop"));
  ...
  r = reader_new("*tests*", " ( ' (23 , 2 ) : 1.5 ) ", 1, 0);
  tk = reader_process(r);
  assert(token_type(tk) == token_LIST);
  a = token_list(tk);
  assert(arr_size(a) == 1);
