...
void stat_tests () {
  ...
  Stat *func = stat_func(exp_pr(exp_sym("a"), arr_new()));
  ...
  Stat *trace = stat_trace(FALSE, exp_sym("a"));
  ...
  Stat *try = stat_try(mk_sc(stat_empty()), "e", mk_sc(stat_empty()), opt_none());
  assert(stat_is_try(try));
  // [<Stat>, <char>, <Stat>, <Opt<Stat>>]
  Arr *try_v = stat_get_try(try);
  assert(stat_is_empty(stat_code_stat(arr_get(try_v, 0))));
  TEST(arr_get(try_v, 1), "e");
  ...
  Stat *try2 = stat_try(
    mk_sc(stat_empty()), "e", mk_sc(stat_empty()),
  ...
  Stat *forv = stat_for("e", exp_array(arr_new()), mk_sc(stat_empty()));
  assert(stat_is_for(forv));
  // [<char>, <Exp>, <Stat>]
  Arr *for_v = stat_get_for(forv);
  TEST(arr_get(for_v, 0), "e");
  ...
  Stat *for_r = stat_for_r("e", exp_int(0), exp_int(10), mk_sc(stat_empty()));
  assert(stat_is_for_r(for_r));
  // [<char>, <Exp>, <Exp>, <Stat>]
  Arr *for_r_v = stat_get_for_r(for_r);
  TEST(arr_get(for_r_v, 0), "e");
  ...
  Stat *for_rs = stat_for_rs(
    "e", exp_int(0), exp_int(10), exp_int(2), mk_sc(stat_empty())
  );
  assert(stat_is_for_rs(for_rs));
  // [<char>, <Exp>, <Exp>, <Exp>, <Stat>]
  Arr *for_rs_v = stat_get_for_rs(for_rs);
  TEST(arr_get(for_rs_v, 0), "e");
  ...
  Stat *switchv = stat_switch(
    ...
    arr_new_from(
      ...
      tp_new(exp_sym("default"), stat_empty()),
      NULL
    )
  );
