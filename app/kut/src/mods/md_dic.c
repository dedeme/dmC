...
static Exp *copy (Arr *exps) {
  ...
  return exp_map((Map *)arr_copy((Arr *)exp_rget_map(arr_get(exps, 0))));
}
...
static Exp *keys (Arr *exps) {
  ...
  return exp_array(arr_map(map_keys(exp_rget_map(arr_get(exps, 0))), (FMAP)fn));
}
...
static Exp *size (Arr *exps) {
  ...
  return exp_int(map_size(exp_rget_map(arr_get(exps, 0))));
}
...
static Exp *to_arr (Arr *exps) {
  ...
  return exp_array(
    arr_map((Arr *)exp_rget_map(arr_get(exps, 0)), (FMAP)f)
  );
}
...
static Exp *values (Arr *exps) {
  ...
  return exp_array(
    arr_map((Arr *)exp_rget_map(arr_get(exps, 0)), (FMAP)kv_value)
  );
}
