...
static Exp *aopen (Arr *exps) {
  ...
  return obj_file(file_aopen(exp_rget_string(arr_get(exps, 0))));
}

// \s -> ()
static Exp *cd (Arr *exps) {
  CHECK_PARS ("file.cd", 1, exps);
  file_cd(exp_rget_string(arr_get(exps, 0)));
  return exp_empty();
}
...
static Exp *close (Arr *exps) {
  ...
  file_close(obj_rget_file(arr_get(exps, 0)));
  return exp_empty();
}
...
static Exp *del (Arr *exps) {
  ...
  file_del(exp_rget_string(arr_get(exps, 0)));
  return exp_empty();
}
...
static Exp *dir (Arr *exps) {
  ...
  return exp_array(
    arr_map(file_dir(exp_rget_string(arr_get(exps, 0))), (FMAP)exp_string)
  );
}
...
static Exp *exists (Arr *exps) {
  ...
  return exp_bool(file_exists(exp_rget_string(arr_get(exps, 0))));
}

// \s -> b
static Exp *is_directory (Arr *exps) {
  CHECK_PARS ("file.isDirectory", 1, exps);
  return exp_bool(file_is_directory(exp_rget_string(arr_get(exps, 0))));
}

// \s -> b
static Exp *is_link (Arr *exps) {
  CHECK_PARS ("file.isLink", 1, exps);
  return exp_bool(file_is_link(exp_rget_string(arr_get(exps, 0))));
}

// \s -> b
static Exp *is_regular (Arr *exps) {
  CHECK_PARS ("file.isRegular", 1, exps);
  return exp_bool(file_is_regular(exp_rget_string(arr_get(exps, 0))));
}

// \s -> ()
static Exp *mkdir (Arr *exps) {
  CHECK_PARS ("file.mkdir", 1, exps);
  file_mkdir(exp_rget_string(arr_get(exps, 0)));
  return exp_empty();
}
...
static Exp *read (Arr *exps) {
  ...
  return exp_string(file_read(exp_rget_string(arr_get(exps, 0))));
}
...
static Exp *read_line (Arr *exps) {
  ...
  char *l = file_read_line(obj_rget_file(arr_get(exps, 0)));
  if (*l) {
    if (l[strlen(l) - 1] == '\n') l = str_left(l, -1);
...
static Exp *ropen (Arr *exps) {
  ...
  return obj_file(file_ropen(exp_rget_string(arr_get(exps, 0))));
}

// \s -> i
static Exp *size (Arr *exps) {
  CHECK_PARS ("file.size", 1, exps);
  return exp_int(file_size(exp_rget_string(arr_get(exps, 0))));
}

// \s -> i
static Exp *tm (Arr *exps) {
  CHECK_PARS ("file.tm", 1, exps);
  return exp_int(file_modified(exp_rget_string(arr_get(exps, 0))));
}
...
static Exp *wopen (Arr *exps) {
  ...
  return obj_file(file_wopen(exp_rget_string(arr_get(exps, 0))));
}
